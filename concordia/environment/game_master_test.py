...
class GameMasterTest(parameterized.TestCase):
  ...
  def test_calls_sequence(self):
    ...
    jane_call_tracker = CallTrackingComponent()

    jane = deprecated_agent.BasicAgent(
        model,
        'Jane',
        clock,
        [jane_call_tracker],
    ...
    with self.subTest('jane'):
      ...
      self.assertEqual(jane_call_tracker.calls_sequence, jane_expected_calls)
