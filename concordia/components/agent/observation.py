...
class Observation(action_spec_ignored.ActionSpecIgnored):
  ...
  def _make_pre_act_value(self) -> str:
    ...
    # """Modified to save the memory bank to a file."""
    # agent_name = self.get_entity().name
    # #scenario_name = self.get_entity().scenario_name()
    # scenario_name = self.get_entity()._context_components['__memory__']._memory._memory._memory_bank.iloc[0].text.replace(' ', '_')
    # self._timeframe_delta_from = datetime.timedelta(hours=1)
    ...
    # console.rule(
    #     f'{agent_name}\'s observations in "{scenario_name}" from {segment_start} to {segment_end}'
    # )
    memory = self.get_entity().get_component(
        self._memory_component_name,
        type_=memory_component.MemoryComponent)
    ...
    # save path is "logs/memory_bank_agentname_%YY-%MM-%DD_%hh-%mm-%ss.jsonl"
    # import os
    # os.makedirs(f"logs/{scenario_name}", exist_ok=True)
    # path = (
    #     f"logs/{scenario_name}/memory_bank_"
