...
from concordia.utils.helper_functions import convert_timestamp_to_time
...
class AssociativeMemoryBank(memory_lib.MemoryBank):
  ...
  def save(self, path: str, **kwargs) -> None:
    ...
    df = self._memory.get_data_frame()
    # create name columns and its values by kwargs
    for key, value in kwargs.items():
        df[key] = value
    # rename column "time" to "timestamp"
    df = df.rename(columns={"time": "timestamp"})
    df["time"] = df["timestamp"].apply(convert_timestamp_to_time)
    # remove column "embedding" with its values
    ...
    if len(kwargs) > 0:
        ...
        columns = ["text", "time", "timestamp", "importance", "tags"]
    df = df[columns]
    # sort records by "timestamp" column
    df = df.sort_values(by="timestamp", ascending=False)
    df.to_json(path_or_buf=path, orient="records", indent=4, force_ascii=False)
    console.log(f'Saved memory to "{path}"')
