...
SETUP_TIME = datetime.datetime(hour=21, year=2025, month=3, day=20)
START_TIME = datetime.datetime(hour=21, year=2025, month=3, day=20)


shared_memories = [
    ]
shared_context = model.sample_text(
    'Summarize the following passage in a concise and insightful fashion.Kindly ensure that all content is presented using proper English punctuation.\n'
...
def make_random_big_five(name: str) -> str:
    if name == 'Alice':
        # 为Alice生成随机的大五人格值
        return str({
...
def get_extras(name, is_main_character, is_staff):
    if not is_staff:
        return {
            'specific_memories': [f'{name} is a student in the dormitory.'],
...
if Use_Previous_profile:
  ...
  numerical_desire = {
  desire_name : int(random.randint(3,4 ))
...
def build_players_list(blank_memory_factory: blank_memories.MemoryFactory,
  ...
  player_configs = [
      formative_memories.AgentConfig(
          name='Alice',
          gender='Female',
          goal="" if current_agent.lower() != 'D2A' else '',
          context=shared_context+"",
          traits = make_random_big_five(name='Alice'),
      ...
      formative_memories.AgentConfig(
          ...
          goal='',
          context=shared_context + "",
          traits = make_random_big_five(name='Jane'),
          extras=get_extras('Jane', False, is_staff=True)
              ),
      formative_memories.AgentConfig(
          name='Charlie',
          gender='Female',
...
def build_game_master(main_character, players, player_names, memories, clock, player_configs, blank_memory_factory):
  ...
  facts_on_village = generic_components.constant.ConstantComponent(
      ' '.join(shared_memories), 'General knowledge of class.'
  )
  ...
  for index, player in enumerate(players):
    ...
    if player.name == 'Jane' or player.name == 'Charlie':
      player.observe(
          f'{player.name} is in the dormitory, {how_to_call} is chatting with others.'
      )
      game_master_memory.add(f'{player.name} is in the dormitory.')
    else:
      player.observe(
          f'{player.name} is a student in the dormitory, {how_to_call} is in the dormitory.'
      )
      game_master_memory.add(f'{player.name} is in the dormitory. People can find {player.name} in the dormitory.')
...
def start_simulation(current_test_agent: str):
  ...
  start=SETUP_TIME,
  step_sizes=[datetime.timedelta(minutes=10), datetime.timedelta(seconds=10)]
  )
